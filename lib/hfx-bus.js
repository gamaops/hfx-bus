...
class HFXBus extends EventEmitter {
	...
	async forward(message) {
		...
		await this._publishMessageEvent(message, 'forwarded');
		return message;
	}

	async _publishMessageEvent(message, event, source = {}) {
		const {
			messageId,
			trackerId,
			groupName,
	...
	hookConsumeMessage({
		...
		this._decorateMessage(message);
		const consume = async (status, unsafe = false) => {
	...
	async stop(force = false) {
		...
		if ('hfxbus' in this.clients) await this.clients.quit();
		this._listeningChannels = new Set();
		this._consumeRunning = {};
		this._scheduledGroups = {};
		this.clients = {};
