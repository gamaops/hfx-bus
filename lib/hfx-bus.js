...
class HFXBus extends EventEmitter {
	constructor(redis, options = {}) {
		...
		this._id = nanoid(HFXBUS_ID_SIZE);
	...
	async forward(message) {
		message.messageId = nanoid(HFXBUS_ID_SIZE);
		const {
			messageId,
			trackerId,
			streamName,
		...
		this.publishMessageEvent(message, 'forwarded').catch((error) => {
	...
	async publishMessageEvent(message, event, source = '') {
		...
		const client = this.getClient();
		const data = `{"gpn":"${groupName}","stn":"${streamName}","mid":"${messageId}","tid":"${trackerId}"${source}}`;
	...
	message(message = {}) {
		if (!('trackerId' in message)) message.trackerId = nanoid(HFXBUS_ID_SIZE);
	...
	_decorateMessage(message) {
		...
		message.load = async (key = 'payload', { decodeJson = false, drop = false, strictDefined = true } = {}) => {
			const client = this.getClient();
			...
			if (message[key] === null) {
				if (strictDefined) {
					throw new errors.UndefinedPayloadError(
						`Undefined payload key "${key}"`,
						{
							key,
							message,
						},
		...
		message.save = async (key = 'payload', { encodeJson = false, expirationTime = null } = {}) => {
			...
			if (expirationTime) saveKeyOptions = ['PX', expirationTime];
	...
	hookConsumeMessage({
		...
		const consume = async (status, unsafe) => {
			...
			if (this._consumingCount[groupName] < this._options.maxParallel) {
				this._forceConsume(groupName);
			}
		};
	...
	_forceConsume(groupName) {
		this._consumeRunning[groupName] = false;
		this.emit(`_consume:${groupName}`);
	}

	_scheduleConsume(groupName) {
		this.removeAllListeners(`_consume:${groupName}`);
		this.on(`_consume:${groupName}`, () => {
			if (!this._consumeRunning[groupName] && this._groupsStreams[groupName]) {
				this._consumeRunning[groupName] = true;
				this._consume(groupName).then(
					(hasMessages) => {
						if (hasMessages === false) this._forceConsume(groupName);
					},
				).catch(
					(error) => {
						this._consumeRunning[groupName] = false;
	...
	async stop(force = false) {
		...
		for (const groupName in this._groupsStreams) this.removeAllListeners(`_consume:${groupName}`);
		this._groupsStreams = {};
		const awaitPending = [];
